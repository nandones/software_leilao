package com.mycompany.clientleilao;

import com.mycompany.view.MainFrame;
import java.net.MulticastSocket;
import java.security.KeyPair;
import java.security.NoSuchAlgorithmException;
import java.security.PrivateKey;
import java.security.PublicKey;
import java.util.Base64;
import java.util.Scanner;
import java.util.logging.Level;
import java.util.logging.Logger;
import javax.crypto.SecretKey;
import javax.crypto.spec.IvParameterSpec;
import javax.swing.SwingUtilities;
import org.json.JSONObject;

/**
 * So, as an evaluation of the computational security matter, there is no need
 * for an elaborate implementation to ensure user authentication, so the finals
 * that specifies the Public Key, Private and CPF will be arranged in the code
 * as a comment, when running this client, it will be necessary to uncomment a
 * set of finals.<br>
 * All keys where generated by CryptoUtils.generateRSAKeyPair().
 *
 * @see ClientCryptoUtils .generateRSAKeyPair()
 * @author nandones
 */
public class ClientAuction {

    public static Scanner input = new Scanner(System.in);

    //user 1:
    //public static final String PUBLIC_KEY_BASE64 = "";
    //public static final String PRIVATE_KEY_BASE64 = "";
    //public static final String CPF = "00000000001";
    //user 2:
    //public static final String PUBLIC_KEY_BASE64 = "";
    //public static final String PRIVATE_KEY_BASE64 = "";
    //public static final String CPF = "00000000002";
    //user 3:
    public static final String PUBLIC_KEY_BASE64 = "";
    public static final String PRIVATE_KEY_BASE64 = "";
    public static final String CPF = "00000000003";

    public static PublicKey publicKey = ClientCryptoUtils.convertBase64StringToPublicKey(PUBLIC_KEY_BASE64);
    public static PrivateKey privateKey = ClientCryptoUtils.convertBase64StringToPrivateKey(PRIVATE_KEY_BASE64);
    public static SecretKey secretKey;
    public static IvParameterSpec iv;
    
    
    public static final String SERVER_PUBLIC_KEY_BASE64 = "";
    public static final PublicKey SERVER_PUBLIC_KEY_BYTES = ClientCryptoUtils.convertBase64StringToPublicKey(SERVER_PUBLIC_KEY_BASE64);
    
    //data for the auction (CPF is in user section)
    public static int multicastPort;
    public static String multicastAddress;
    public static MulticastSocket multicastSocket;
    
    public static boolean userKnowsItem = false;//used to solicitate the item details
    public static final String USER = "client";
    public static String itemName;
    public static String itemDescription;
    public static float currentPrice;
    public static float bidIncrement;
    public static int chronometer;
    public static float nextbid;
    
    
    
    

    public static void main(String[] args) throws Exception {
        //testingAES();
        //testingSignature();
        //testingRSA();
        startJFrame();
        //testingUsers();
        //testingJson();
        //connectsToMulticast("192.168.3.15", 12345);
    }

    public static void testingAES() throws Exception {
        String plaintext = "Why in the world would you hit me when im down? when im down.";
        SecretKey key = ClientCryptoUtils.generateAESKey();
        String secretKeySTR = ClientCryptoUtils.convertAESKeyToBase64(key);
        System.out.println("secretkey: " + secretKeySTR);
        IvParameterSpec iv = ClientCryptoUtils.generateIvFromAESKey(key);
        String ciphertext = ClientCryptoUtils.encryptWithAES(plaintext, key, iv);
        key = ClientCryptoUtils.convertBase64ToSecretKey(secretKeySTR);
        System.out.println("ciphertext = " + ciphertext);
        System.out.println("texto descriptografado: " + ClientCryptoUtils.decryptWithAES(ciphertext, key, iv));
    }

    public static void testingSignature() throws NoSuchAlgorithmException, Exception {
        //sender
        String message = "who could it be now?🎷🎷🎷";
        KeyPair keypair = ClientCryptoUtils.generateRSAKeyPair();
        PublicKey puk = keypair.getPublic();
        String pukSTR = ClientCryptoUtils.convertPublicKeyToBase64String(puk);
        System.out.println("public key1  : " + pukSTR);
        PrivateKey prk = keypair.getPrivate();
        String prkSTR = ClientCryptoUtils.convertPrivateKeyToBase64String(prk);
        System.out.println("private key1 : " + prkSTR);
        ClientCryptoUtils.hashMessage(message);
        byte[] signatureBytes = ClientCryptoUtils.signMessage(message, prk);
        //receiver
        PublicKey puk2 = ClientCryptoUtils.convertBase64StringToPublicKey(pukSTR);
        PrivateKey prk2 = ClientCryptoUtils.convertBase64StringToPrivateKey(prkSTR);
        pukSTR = ClientCryptoUtils.convertPublicKeyToBase64String(puk2);
        System.out.println("public key2  : " + pukSTR);
        prkSTR = ClientCryptoUtils.convertPrivateKeyToBase64String(prk2);
        System.out.println("private key2 : " + prkSTR);
        System.out.println(ClientCryptoUtils.verifySignature(message, signatureBytes, puk2));
        System.out.println(prk.equals(prk2));
        System.out.println(puk.equals(puk2));
        // a new plus test
        System.out.println("insira a public key: ");
        String puk3STR = input.nextLine();
        System.out.println(pukSTR);
        PublicKey puk3 = ClientCryptoUtils.convertBase64StringToPublicKey(puk3STR);
        System.out.println(puk.equals(puk3));
    }

    public static void testingRSA() throws NoSuchAlgorithmException, Exception {
        //sender
        KeyPair keyPair = ClientCryptoUtils.generateRSAKeyPair();
        PublicKey puk = keyPair.getPublic();
        PrivateKey prk = keyPair.getPrivate();
        String plainText = "i always feels like somebody is watchiiing meee";
        String cipherText = ClientCryptoUtils.encryptWithRSA(plainText, puk);
        String pukSTR = ClientCryptoUtils.convertPublicKeyToBase64String(puk);
        String prkSTR = ClientCryptoUtils.convertPrivateKeyToBase64String(prk);
        //receiver
        PublicKey puk2 = ClientCryptoUtils.convertBase64StringToPublicKey(pukSTR);
        PrivateKey prk2 = ClientCryptoUtils.convertBase64StringToPrivateKey(prkSTR);
        System.out.println(ClientCryptoUtils.decryptWithRSA(cipherText, prk2));
        System.out.println("pukSTR:");
        System.out.println(pukSTR);
        System.out.println("prkSTR:");
        System.out.println(prkSTR);

    }

    public static void startJFrame() {
        SwingUtilities.invokeLater(() -> {
            MainFrame mainFrame = new MainFrame();
            mainFrame.setVisible(true); // Torna o JFrame visível
        });
    }

    public static void testingUsers() {
        //sender
        String plainText = "nunca tenha medo do seu inimigo quando não é você que começa a brigar 🎵";
        String cipherText = ClientCryptoUtils.encryptWithRSA(plainText, publicKey);
        //reciver
        System.out.println(ClientCryptoUtils.decryptWithRSA(cipherText, privateKey));
    }

    public static void testingJson() {
        try {
            //client
            String plaintext = "cowboys must shoot fast";
            byte[] signatureSNDR = ClientCryptoUtils.signMessage(plaintext, privateKey);
            String signatureBase64 = Base64.getEncoder().encodeToString(signatureSNDR);
            String jsonPayload = ClientCommunicationUtils.createJoinRequestJson(CPF, plaintext, signatureBase64);
            System.out.println("client: " + jsonPayload);
            //server receiving
            JSONObject joinRequest = new JSONObject(jsonPayload);
            String CPF = joinRequest.getString("CPF");
            String text = joinRequest.getString("text");
            String signatureRCVR = joinRequest.getString("signature");
            System.out.println("CPF: " + CPF);
            System.out.println("text: " + text);
            System.out.println("signatureRCVR: " + signatureRCVR);
            //server checking signature
            byte[] signatureBytes = Base64.getDecoder().decode(signatureBase64);
            //he will gwt the public key from cpf, in a map i guess.
            boolean signatureBOOL = ClientCryptoUtils.verifySignature(text, signatureBytes, publicKey);
            System.out.println("user authenticated? " + signatureBOOL);
        } catch (Exception ex) {
            Logger.getLogger(ClientAuction.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    /**
     * 
     * @param serverIP
     * @param ServerPort
     * @return a boolean about the connection.
     */
    public static boolean connectsToMulticast(String serverIP, int ServerPort) {
        try {
            String plaintext = "join request";
            byte[] signatureBytes = ClientCryptoUtils.signMessage(plaintext, privateKey);
            String signatureBase64 = Base64.getEncoder().encodeToString(signatureBytes);
            System.out.println(ClientCryptoUtils.convertPublicKeyToBase64String(publicKey));
            String responseJsonSTR = ClientCommunicationUtils.sendRequestToServer(CPF, plaintext, signatureBase64, serverIP, ServerPort);
            System.out.println(responseJsonSTR);
            //data extraction
            JSONObject responseJson = new JSONObject(responseJsonSTR);
            
            multicastAddress = responseJson.getString("adress");
            multicastPort = responseJson.getInt("port");
            String encodedSecretKey = responseJson.getString("secretKey");////String in base64, encoded with client public key.
            String text = responseJson.getString("text");
            String serverSignature = responseJson.getString("signature");
            
            System.out.println("multicastAdress : "+ multicastAddress);
            System.out.println("multicastPort : "+ multicastPort);
            System.out.println("encodedSecretKey : "+ encodedSecretKey);
            System.out.println("text : "+ text);
            System.out.println("serverSignature : "+ serverSignature);
            //authenticate server
            signatureBytes = Base64.getDecoder().decode(serverSignature);
            System.out.println(ClientCryptoUtils.verifySignature(text, signatureBytes, SERVER_PUBLIC_KEY_BYTES));
            //decodes the secret key
            String secretKeyBase64 = ClientCryptoUtils.decryptWithRSA(encodedSecretKey, privateKey);
            secretKey = ClientCryptoUtils.convertBase64ToSecretKey(secretKeyBase64);
            iv = ClientCryptoUtils.generateIvFromAESKey(secretKey);
            
        } catch (Exception ex) {
            Logger.getLogger(ClientAuction.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
        return true;
    }
}
